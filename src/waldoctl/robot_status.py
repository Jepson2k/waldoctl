...
class ChangeNotifierMixin:
    """Coarse-grained change notification for cases ``bindable_dataclass`` can't
    express on its own.
    ...
    The listener list is built lazily on first registration, so subclasses do
    not need to redeclare it as a dataclass field. Copy-on-write storage lets
    ``notify_changed`` iterate safely while new listeners are being added.
    """
